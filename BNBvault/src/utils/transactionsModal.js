...
import {
  ...
  MultiOwnerAccountAbstractionFactory,
} from './constants';
...
import {abiMOAAFactory} from '../contracts/moaaFactory';
...
export default class TransactionsModal extends Component {
  ...
  async checkTransaction() {
    ...
    if (this.context.value.transactionData.command === 'transfer') {
      ...
      const multiUserAccountAbstraction = new ethers.Contract(
        MultiOwnerAccountAbstractionFactory,
        abiMOAAFactory,
        this.provider,
      );
      const transaction =
        await multiUserAccountAbstraction.populateTransaction.createAccount(
          [this.context.value.publicKey, CloudAccountController],
          0,
          {
            from: signer.address,
          },
    ...
    } else if (this.context.value.transactionData.command === 'transferCard') {
      const contractMOAA = new ethers.Contract(
        this.context.value.publicKeyCard,
        abiMOAAContract,
        this.provider,
      );
      const transaction = await contractMOAA.populateTransaction.addDeposit({
        from: signer.address,
        value: ethers.utils.parseEther(
          this.context.value.transactionData.amount,
        ),
  ...
  async processTransaction() {
    ...
    if (this.context.value.transactionData.command === 'transfer') {
      ...
      const tx = await signer.sendTransaction(transaction);
      await tx.wait();
      const publicKeyCard = await multiUserAccountAbstraction.getAddress(
        [this.context.value.publicKey, CloudAccountController],
        0,
      );
